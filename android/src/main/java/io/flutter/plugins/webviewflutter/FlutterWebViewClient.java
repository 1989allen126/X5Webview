...
class FlutterWebViewClient {
  ...
  private static String errorCodeToString(int errorCode) {
    switch (errorCode) {
      ...
      case WebViewClient.ERROR_UNKNOWN:
        return "unknown";
      case WebViewClient.INTERCEPT_BY_ISP:
        return "interceptByIsp";
  ...
  WebViewClient createWebViewClient(boolean hasNavigationDelegate,Context context) {
    ...
    if (!hasNavigationDelegate || android.os.Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
      return internalCreateWebViewClient(context);
    }

    return internalCreateWebViewClientCompat(context);
  }

  private WebViewClient internalCreateWebViewClient(Context context) {
    return new WebViewClient() {
      @Override
      public boolean shouldOverrideUrlLoading(WebView view, WebResourceRequest request) {
        String scheme = request.getUrl().getScheme().trim();
        Log.d("WebViewClient","===========scheme:" + scheme);
        if(!scheme.isEmpty() && (!scheme.startsWith("http") && !scheme.startsWith("https"))) {
          startActivity(context, request.getUrl());
          return false;
        }
      ...
      public boolean shouldOverrideUrlLoading(WebView view, String url) {
        ...
        Log.d("WebViewClient","===========scheme:" + scheme);
        if(!scheme.isEmpty() && (!scheme.startsWith("http") && !scheme.startsWith("https"))) {
          startActivity(context,Uri.parse(url));
          return false;
        }
  ...
  private WebViewClient internalCreateWebViewClientCompat(Context context) {
    return new WebViewClient() {
      ...
      @Override
      public void onReceivedError(WebView var1, WebResourceRequest request, WebResourceError error){
        FlutterWebViewClient.this.onWebResourceError(
            error.getErrorCode(), error.getDescription().toString(), request.getUrl().toString());
      }
